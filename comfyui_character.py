...
class ComfyUICharacter:
    ...
    def _generate(
        ...
        with torch.inference_mode():
            ...
            if image_url:
                decoded = self._apply_face_pipeline(decoded, ref_image, pos, neg, face_description)
    ...
    def _pick_face(self, segs, decoded, face_description=""):
        """Pick a face from detected segments.

        If face_description is provided and the VLM is loaded, uses vision to
        pick the matching face. Otherwise falls back to the largest face.
        """
        seg_header, seg_list = segs
        ...
        try:
            return self._pick_face_with_vision(segs, decoded, face_description)
        except Exception as e:
            self.logger.warning(f"Vision face picking failed: {e}, falling back to largest face")
            return self._pick_largest_face(segs)

    def _pick_face_with_vision(self, segs, decoded, face_description):
        """Use Qwen2.5-VL vision model to pick the face matching a description.

        Draws numbered bounding boxes on the generated image, sends it to
        ...
        # Convert decoded tensor to PIL image with numbered bounding boxes
        img_array = decoded[0].detach().cpu().numpy()
        img_array = (img_array * 255).clip(0, 255).astype(np.uint8)
        img = Image.fromarray(img_array)
        ...
        if not match:
            self.logger.warning(f"Could not parse face number from LLM response: {answer}")
            return self._pick_largest_face(segs)

        face_idx = int(match.group()) - 1  # Convert 1-based to 0-based
        if face_idx < 0 or face_idx >= len(seg_list):
            self.logger.warning(f"LLM picked invalid face {face_idx + 1}/{len(seg_list)}")
            return self._pick_largest_face(segs)
    ...
    def _apply_face_pipeline(self, decoded, ref_image, pos, neg, face_description=""):
        ...
        # Step 2: Pick face (vision model if description provided, else largest)
        picked_segs = self._pick_face(segs, decoded, face_description)
